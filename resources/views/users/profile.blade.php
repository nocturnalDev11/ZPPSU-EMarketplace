...
@section('content')
    <div class="flex container transition-all h-full xl:ml-80 lg:ml-80 md:w-full w-full p-4 xl:pt-16 pt-24 mx-auto">
            ...
            <main>
                <div class="grid grid-cols-1 px-4 pt-6 xl:grid-cols-3 xl:gap-6">
                        ...
                        <nav class="flex mb-5" aria-label="Breadcrumb">
                            <ol class="inline-flex items-center space-x-1 md:space-x-2">
                                <li class="inline-flex items-center">
                                    <a href="{{ route('users.home') }}" wire:navigate class="inline-flex items-center text-gray-700 hover:text-gray-900 dark:text-gray-50 dark:hover:text-white">
                                        <svg class="w-5 h-5 mr-2.5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M10.707 2.293a1 1 0 00-1.414 0l-7 7a1 1 0 001.414 1.414L4 10.414V17a1 1 0 001 1h2a1 1 0 001-1v-2a1 1 0 011-1h2a1 1 0 011 1v2a1 1 0 001 1h2a1 1 0 001-1v-6.586l.293.293a1 1 0 001.414-1.414l-7-7z"></path>
                                        </svg>
                                        <span class="ml-1 text-sm font-medium text-gray-400 md:ml-2" aria-current="page">Home</span>
                                    </a>
                                </li>
                                <li>
                                    <div class="flex items-center">
                                        ...
                                        <p class="ml-1 md:ml-2 text-sm font-medium text-gray-700 dark:text-gray-50">Profile</p>
                                    </div>
                                </li>
                            </ol>
                        </nav>
                    ...
                    <div class="col-span-full xl:col-auto">
                        <div class="bg-white shadow-lg shadow-gray-200 dark:shadow-gray-700/50 rounded-2xl p-4 mb-6 dark:bg-gray-800">
                            <div class="sm:flex xl:block sm:space-x-4 xl:space-x-0">
                                @if($user->profile_picture)
                                    <img class="mb-2 w-20 h-20 rounded-2xl shadow-lg shadow-gray-300 dark:shadow-gray-700 object-cover" src="{{ asset('storage/' . $user->profile_picture) }}" alt="{{ $user->first_name }}'s picture" />
                                @else
                                    <div class="relative inline-flex items-center justify-center rounded-2xl shadow-lg shadow-gray-300 dark:shadow-gray-700 mb-2 overflow-hidden bg-gray-200 dark:bg-gray-600 w-20 h-20">
                                ...
                                @endif
                                <div>
                                    <h2 class="text-xl font-bold dark:text-gray-200">{{ $user->first_name }} {{ $user->last_name }}</h2>
                                    <ul class="mt-2 space-y-1">
                                        <li class="flex items-center text-md font-normal text-gray-500 dark:text-gray-200">
                                            <svg class="mr-2 w-4 h-4 text-gray-900 dark:text-gray-200" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <div class="sm:flex-1">
                                    <address class="text-sm not-italic font-normal text-gray-500">
                                        ...
                                        <div class="text-md font-medium text-gray-900 dark:text-gray-200">
                                            @if (empty($user->email))
                                                {{ $user->first_name }} has not provided an email yet.
                                            @else
                                                {{ $user->email }}
                                            @endif
                                        </div>
                                        <div class="mt-4 dark:text-gray-300">Role</div>
                                    ...
                                    <li class="flex flex-col py-2">
                                        ...
                                        @if (empty($user->about))
                                            <span class="text-gray-700 dark:text-gray-300">
                                                {{ $user->first_name }} has not provided an about.
                                            </span>
                                        @else
                        ...
                        <div class="bg-white shadow-lg shadow-gray-200 dark:shadow-gray-700/50 rounded-2xl p-4 mb-6 dark:bg-gray-800">
                            <div class="flow-root">
                                <h3 class="text-xl font-bold dark:text-gray-200">{{ $user->first_name }}'s activity</h3>
                                <div class="relative px-4">
                                    ...
                                    @else
                                        <div class="absolute h-full border-dashed border border-opacity-20 border-gray-600 dark:border-gray-200"></div>
                    ...
                    <div class="col-span-2">
                        @foreach($user->products as $product)
                            <div class="flex flex-col lg:flex-row items-center bg-white shadow-lg shadow-gray-200 dark:shadow-gray-700/50 dark:bg-gray-800 rounded-2xl p-4 mb-6">
                                <img class="object-cover w-full lg:w-48 h-96 lg:h-48 rounded-lg" src="{{ asset('storage/' . $product->prod_picture) }}" alt="{{ $product->prod_name }}">
                                <div class="flex flex-col w-full justify-between p-4 leading-normal">
                                    <h5 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ $product->prod_name }}</h5>
                                    <p class="font-normal text-green-600 dark:text-green-400">₱{{ number_format($product->prod_price, 2) }}</p>
                                    <div class="my-2">
                                    ...
                                    <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">{{ $product->prod_description }}</p>
                                </div>
                            </div>
                        @endforeach
                        ...
                        @foreach($user->services as $service)
                            <div class="flex flex-col lg:flex-row items-center bg-white shadow-lg shadow-gray-200 dark:shadow-gray-700/50 dark:bg-gray-800 rounded-2xl p-4 mb-6">
                                @if ($service->services_picture)
                                    <img class="object-cover w-full lg:w-48 h-96 lg:h-48 rounded-lg" src="{{ asset('storage/' . $service->services_picture) }}" alt="{{ $service->services_title }}">
                                @else
                                    <div class="relative flex items-center justify-center overflow-hidden rounded-2xl bg-cover bg-center w-full xl:w-48 h-96 xl:h-48 shadow-soft-xl">
                                ...
                                @endif
                                <div class="flex flex-col w-full justify-between p-4 leading-normal">
                                    <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ $service->services_title }}</h5>
                                    <p class="font-normal text-green-600 dark:text-green-400">₱{{ number_format($service->services_fee, 2) }}</p>
                                    <div class="my-2">
                                        <span class="inline-block bg-gray-100 text-gray-700 rounded-full px-3 py-1 text-sm font-semibold dark:bg-gray-600 dark:text-gray-300">
                                            Category: {{ $service->services_category }}
                                        </span>
                                    </div>
                                    <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">{!! nl2br(e($service->services_description)) !!}</p>
                                </div>
                            </div>
                        @endforeach
                        ...
                        @foreach($user->posts as $post)
                            <div class="flex flex-col lg:flex-row items-center bg-white shadow-lg shadow-gray-200 dark:shadow-gray-700/50 dark:bg-gray-800 rounded-2xl p-4 mb-6">
                                @if ($post->post_picture)
                                    <img class="object-cover w-full lg:w-48 h-96 lg:h-48 rounded-lg" src="{{ asset('storage/' . $post->post_picture) }}" alt="{{ $post->post_title }}">
                                @else
                                    <div class="relative flex items-center justify-center overflow-hidden rounded-2xl bg-cover bg-center w-full lg:w-48 h-96 lg:h-48 shadow-soft-xl">
                                ...
                                <div class="flex flex-col w-full justify-between p-4 leading-normal">
                                    ...
                                    <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">{!! nl2br(e($post->post_content)) !!}</p>
                                </div>
                            </div>
                        @endforeach
                        ...
                        @foreach($user->trades as $trade)
                            <div class="flex flex-col lg:flex-row items-center bg-white shadow-lg shadow-gray-200 dark:shadow-gray-700/50 dark:bg-gray-800 rounded-2xl p-4 mb-6">
                                @if($trade->trade_picture)
                                    <img class="object-cover w-full lg:w-48 h-96 lg:h-48 rounded-lg" src="{{ asset('storage/' . $trade->trade_picture) }}" alt="{{ $trade->trade_title }}">
                                @else
                                    <div class="relative flex items-center justify-center overflow-hidden rounded-2xl bg-cover bg-center w-full lg:w-48 h-96 lg:h-48 shadow-soft-xl">
                                ...
                                @endif
                                <div class="flex flex-col w-full justify-between p-4 leading-normal">
                                    <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ $trade->trade_title }}</h5>
                                    <p class="font-normal text-green-600 dark:text-green-400">₱{{ number_format($trade->trade_value, 2) }}</p>
                                    <div class="mt-2">
                                    ...
                                    <div class="my-2">
                                        ...
                                        <p class="font-normal text-gray-700 dark:text-gray-400">{!! nl2br(e($trade->trade_conditions)) !!}</p>
                                        <p class="font-semibold text-gray-700 dark:text-gray-400">Description</p>
                                        <p class="font-normal text-gray-700 dark:text-gray-400">{!! nl2br(e($trade->trade_description)) !!}</p>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                ...
                <div x-data="{ open: false }" class="fixed end-6 bottom-6 group">
                    ...
                    <button @click="open = !open"
                        ...
                        <svg class="w-5 h-5 transition-transform" :class="{'rotate-45': open, 'rotate-0': !open}" aria-hidden="true"
