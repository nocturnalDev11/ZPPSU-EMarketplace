...
@section('content')
    ...
    @include('lists.products.delete-modal')
    ...
    <div class="container h-full xl:ml-80 lg:ml-80 md:ml-0 ml-0 p-4 pt-20">
                ...
                <li aria-current="page">
                    <a href="{{ route('lists.products.index') }}" class="inline-flex items-center text-md font-medium text-gray-700 hover:text-maroon">
                        <div class="flex items-center">
                            ...
                            <span class="ms-1 text-md font-medium text-gray-700 md:ms-2">All products</span>
                        </div>
                    </a>
                </li>
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ms-1 text-md font-medium text-gray-500 md:ms-2">Product details</span>
                    </div>
                </li>
            </ol>
        </nav>
        <div class="flex flex-col items-center p-8 xl:flex-row lg:flex-row md:flex-col bg-white shadow-md rounded-md xl:w-full">
            ...
            <div class="xl:pt-0 lg:pt-0 md:pt-8 pt-8 xl:w-2/5 lg:w-2/5 md:w-full w-full">
                <img src="{{ asset('storage/' . $product->prod_picture) }}" alt="{{ $product->prod_name }}" class="object-cover aspect-square rounded-md shadow-md" />
            </div>

            <!-- Product Details -->
            <div class="xl:w-3/5 lg:w-3/5 md:w-full w-full xl:px-12 lg:px-10 md:px-6 px-6 md:mt-0">
                <h1 class="text-3xl font-bold text-gray-800 dark:text-gray-200">{{ $product->prod_name }}</h1>
                <p class="text-xl font-semibold text-gray-700 dark:text-gray-300 mt-2">₱{{ number_format($product->prod_price, 2) }}</p>
                <div class="mt-4">
                ...
                <div class="my-5">
                    ...
                    <div class="flex items-center gap-4 my-2">
                        @if($product->user->profile_picture)
                            <img src="{{ asset('storage/' . $product->user->profile_picture) }}" alt="User Image" class="w-10 h-10 rounded-full me-3">
                        @else
                            <div class="relative inline-flex items-center justify-center w-10 h-10 me-3 overflow-hidden bg-gray-100 rounded-full dark:bg-gray-600">
                                <span class="font-medium text-gray-600 dark:text-gray-300">
                                    {{ strtoupper(substr($product->user->first_name, 0, 1)) }}{{ strtoupper(substr($product->user->last_name, 0, 1)) }}
                                </span>
                            </div>
                        @endif
                        <div class="font-medium dark:text-white">
                            <a href="{{ route('profile.show', ['id' => $product->user->id]) }}" wire:navigate class="cursor-pointer">
                                <span>{{ $product->user->first_name }} {{ $product->user->last_name }}</span>
                            </a>
                            <div class="text-sm text-gray-500 dark:text-gray-400">Joined in {{ date('M Y', strtotime($product->user->created_at)) }}</div>
                        </div>
                    </div>
                    @if(Auth::check() && Auth::id() !== $product->user->id)
                        <div class="flex gap-2">
                            <a href="{{ route('profile.show', ['id' => $product->user->id]) }}" wire:navigate class="w-[200px] px-3 py-2 text-xs font-medium text-center inline-flex items-center justify-center text-gray-700 dark:text-white bg-gray-200 rounded-lg hover:bg-gray-300 focus:outline-none focus:ring-0 dark:bg-gray-600 dark:hover:bg-gray-700">
                    ...
                    @if(Auth::id() === $product->user->id)
                        <div class="flex gap-2">
                            <a href="{{ route('products.edit', $product->id) }}" type="button" class="w-[200px] px-3 py-2 text-xs font-medium text-center inline-flex items-center justify-center text-gray-700 dark:text-white bg-gray-200 rounded-lg hover:bg-gray-300 focus:outline-none focus:ring-0 dark:bg-gray-600 dark:hover:bg-gray-700">
                                <svg class="w-3 h-3 text-gray-700 dark:text-white me-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                ...
                @if(Auth::check() && Auth::id() !== $product->user->id)
                    <form action="{{ route('messages.store') }}" method="POST" enctype="multipart/form-data" class="xl:w-4/5 lg:w-4/5 md:w-full w-full">
                        @csrf
                        <span>Send message to {{ $product->user->first_name }}</span>
                        <input type="hidden" name="recipient_id" value="{{ $product->user->id }}">
                        <input type="hidden" name="content_link" value="{{ route('products.show', $product->id) }}">
                        <input type="hidden" name="content_link_image" value="{{ $product->prod_picture }}">
                        <input type="hidden" name="content_link_description" value="{{ $product->prod_description }}">
        ...
        <!-- Related Products -->
        <div class="container h-full xl:ml-80 lg:ml-80 md:ml-0 ml-0 p-4 pt-20 mx-auto my-6 flex flex-col px-6 py-3 min-w-0 mb-6 break-words bg-gradient-to-br from-red-50 via-purple-50 to-gray-100 border-0 dark:bg-gray-950 dark:shadow-soft-dark-xl shadow-soft-xl rounded-lg bg-clip-border">
            <div class="p-4 pb-0 mb-0 rounded-t-2xl">
                <h2 class="mb-1 text-xl dark:text-white">Related products</h2>
            </div>
                        ...
                        <div class="relative flex flex-col min-w-0 break-words bg-transparent border-0 shadow-none dark:shadow-soft-dark-lg rounded-2xl bg-clip-border">
                            <div class="relative">
                                <a class="block shadow-lg rounded-2xl">
                                    <img src="{{ asset('storage/' . $relatedProduct->prod_picture) }}" alt="{{ $relatedProduct->prod_name }}" class="object-cover bg-center h-44 w-full shadow-soft-xl rounded-2xl" />
                                </a>
                            </div>
                            <div class="flex-auto px-1 pt-6">
                                <a href="{{ route('products.show', $relatedProduct->id) }}" wire:navigate>
                                    <h5 class="dark:text-white">{{ $relatedProduct->prod_name }}</h5>
                                </a>
                                ...
                                <p class="mb-6 leading-normal text-sm dark:text-white dark:opacity-60 text-truncate">{{ $relatedProduct->prod_description }}</p>
                                <div class="flex items-center justify-between">
                                    <a href="{{ route('products.show', $relatedProduct->id) }}" wire:navigate class="inline-block px-8 py-2 mb-0 font-bold text-center uppercase align-middle transition-all text-white bg-gradient-to-r from-purple-500 to-pink-500 hover:bg-gradient-to-l rounded-lg shadow-none cursor-pointer leading-pro ease-soft-in text-xs hover:scale-102 active:shadow-soft-xs tracking-tight-soft border-fuchsia-500 text-fuchsia-500 hover:border-fuchsia-500 hover:bg-transparent hover:text-fuchsia-500 hover:opacity-75 hover:shadow-none active:bg-fuchsia-500 active:text-white active:hover:bg-transparent active:hover:text-fuchsia-500">
                                        View product
                                    </a>
                                </div>
                            </div>
