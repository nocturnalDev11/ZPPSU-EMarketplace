<div x-data="{
    ...
    <div class="flex flex-col items-center justify-between p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
        ...
        <!-- Actions -->
        <div
            class="flex items-center justify-center w-full md:w-auto md:flex-row md:space-y-0 xl:gap-0 gap-2 md:items-center md:space-x-3">
            <!-- Add trade -->
                            ...
                            <form action="{{ route('trades.store') }}" method="POST" enctype="multipart/form-data"
                                ...
                                <div x-data="{ photoName: null, photoPreview: null }"
                                    ...
                                    <label class="block text-gray-700 text-sm font-bold mb-2 text-center" for="photo">
                                        Trading picture
                                        @error('trade_picture') <span class="text-red-600">{{ $message }}</span>
                                        @enderror
                                    </label>
                                ...
                                <div class="grid gap-2 mb-4 grid-cols-2">
                                    ...
                                    <div class="col-span-2">
                                        ...
                                        @error('trade_category') <span class="text-red-600">{{ $message }}</span>
                                        @enderror
                                    </div>

                                    <div class="col-span-2">
                                        <label for="trade_description"
                                            class="block text-sm text-gray-700">Description</label>
                                        ...
                                        @error('trade_description') <span class="text-red-600">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="col-span-2 sm:col-span-1">
                                        ...
                                        @error('trade_status') <span class="text-red-600">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="col-span-2">
                                        <label for="trade_conditions"
                                            class="block text-sm text-gray-700">Conditions</label>
                                        <textarea id="trade_conditions" name="trade_conditions" rows="4"
                                            class="block w-full px-3 py-2 mt-2 text-gray-600 placeholder-gray-400 bg-white border border-gray-200 rounded-md focus:border-indigo-400 focus:outline-none focus:ring focus:ring-indigo-300 focus:ring-opacity-40"></textarea>
                                        @error('trade_conditions') <span class="text-red-600">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="col-span-2">
                                        ...
                                        <input id="trade_duration" name="trade_duration" type="date"
                                            class="block w-full px-3 py-2 mt-2 text-gray-600 placeholder-gray-400 bg-white border border-gray-200 rounded-md focus:border-indigo-400 focus:outline-none focus:ring focus:ring-indigo-300 focus:ring-opacity-40">
                                        @error('trade_duration') <span class="text-red-600">{{ $message }}</span>
                                        @enderror
                                    </div>
                                </div>
            ...
            <div @click.away="openSort = false" class="relative">
                ...
                <div x-show="openSort"
                    class="absolute mt-2 w-44 bg-white dark:bg-gray-800 dark:text-gray-200 shadow rounded z-10">
            ...
            <div @click.away="openFilter = false" class="relative">
                ...
                <div x-show="openFilter" class="absolute mt-2 w-48 p-3 bg-white dark:bg-gray-800 shadow rounded z-10">
                    <h6 class="mb-3 text-sm font-medium text-gray-900 dark:text-gray-200">
                        Category
                    </h6>
                    <ul class="space-y-2 text-sm">
                        @foreach (['Apparel', 'School supplies', 'Footwares', 'Stationery', 'Electronics', 'University
                        merchandise', 'Musical instruments', 'Books'] as $category)
            ...
            <div @click.away="openStatus = false" class="relative">
                ...
                <div x-show="openStatus" class="absolute mt-2 w-48 p-3 bg-white dark:bg-gray-800 shadow rounded z-10">
                    ...
                    <ul class="space-y-2 text-sm">
                        @foreach (['Available', 'Pending', 'In-Progress', 'Completed', 'Cancelled', 'Expired',
                        'Negotiating', 'Closed'] as $status)
                        <li class="flex items-center">
                            <input type="checkbox" wire:model.live="selectedStatus" value="{{ $status }}"
                                class="w-4 h-4 bg-gray-100 dark:bg-gray-600 border-gray-300 dark:border-gray-500 rounded text-purple-600 dark:text-gray-200 focus:ring-purple-500">
                            <label for="{{ $status }}"
                                class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-200">
                                {{ $status }}
                ...
                <div
                    ...
                    <div class="relative">
                        <a class="block shadow-lg rounded-2xl">
                            <img src="{{ asset('storage/' . $trade->trade_picture) }}" alt="{{ $trade->trade_title }}"
                    ...
                    <div
                        ...
                        <svg class="w-8 h-8" xmlns="http://www.w3.org/2000/svg" width="24" height="24" aria-hidden="true"
                            viewBox="0 0 24 24">
                            <defs>
                    ...
                    <div class="flex items-center justify-between mt-auto">
                        <a href="{{ route('trades.show', $trade->id) }}" wire:navigate
                            class="inline-block px-8 py-2 mb-0 font-bold text-center uppercase align-middle transition-all text-white bg-gradient-to-br from-green-400 to-blue-600 hover:bg-gradient-to-bl rounded-lg shadow-none cursor-pointer leading-pro ease-soft-in text-xs hover:scale-102 active:shadow-soft-xs tracking-tight-soft border-fuchsia-500 text-fuchsia-500 hover:border-fuchsia-500 hover:bg-transparent hover:text-fuchsia-500 hover:opacity-75 hover:shadow-none active:bg-fuchsia-500 active:text-white active:hover:bg-transparent active:hover:text-fuchsia-500">
                            View trading
                        </a>
                    </div>
                </div>
